    ...
    >
      <span class="custom-tree-node" slot-scope="{ data }">
        <span :title="data[defaultProps.label]">
          <i v-if="data.children" class="treeIcon parentIcon"></i>
          <i v-else class="treeIcon childIcon"></i>
...
export default {
  ...
  methods: {
    ...
    menuClick(callback) {
      ...
      let currentNode = this.$refs[this.treeId].$refs.el.getCurrentNode(
        currentKey
      );
...
.treeContainer {
  ...
  .el-tree {
    ...
    /deep/ .el-tree-node__expand-icon.expanded.el-icon-caret-right {
      background: url(./images/tree_icon.png) no-repeat -30px -272px;
      &:before {
        content: "";
      }
    }
    /deep/ .el-tree-node__expand-icon.el-icon-caret-right {
      width: 5px;
      height: 5px;
      background: url(./images/tree_icon.png) no-repeat -30px -241px;
      &.is-leaf {
        display: none;
      }
      &:before {
